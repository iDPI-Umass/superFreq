<script lang="ts">
	...
	let displayDiscoverFeed = $derived(
		firehoseFeedItems && firehoseFeedItems.length > feedItems.length ? true : false
	);
	...
	{#each feedItems as item}
		<div class="feed-item">
			...
			{:else if item?.item_type == 'comment' && !item?.collection_id}
				<a
					href={`/posts/${item.parent_post_username}/now-playing/${parseTimestamp(item.parent_post_created_at)}#${item.username?.concat(parseTimestamp(item.timestamp))}`}
				>
					<div class="feed-item">
						<div class="feed-item-two-liner-user-row">
							<CoverArt
								...
								imgClass="feed-avatar"
							></CoverArt>
							{item.user_id == sessionUserId ? 'You' : item.display_name} commented on {item.parent_post_user_id ==
							sessionUserId
								? 'your'
								: item.parent_post_display_name.concat(`'s`)} post {item.artist_name ||
							item.user_added_artist_name
								? 'about'
								: ''}
						</div>
						{@render feedItemTag(item)}
				...
				<div class="feed-post-spacer">
					<div class="feed-item-now-playing">
						<PostReply reply={item} {sessionUserId} {userActionSuccess}></PostReply>
					</div>
				</div>
					...
					<div class="feed-item">
						<div class="feed-item-two-liner-user-row">
							<CoverArt
								...
								imgClass="feed-avatar"
							></CoverArt>
							{item.user_id == sessionUserId ? 'You' : item.display_name} replied to {item.parent_post_user_id ==
							sessionUserId
								? 'your'
								: item.parent_post_display_name.concat(`'s`)} comment on a post {item.artist_name ||
							item.user_added_artist_name
								? 'about'
								: ''}
						</div>
			...
			{:else if item?.item_type == 'comment' && item?.collection_id}
				<a
					href={`/collection/${item.collection_id}#${item.username?.concat(parseTimestamp(item.timestamp))}`}
				>
					<div class="feed-item">
						<div class="feed-item-two-liner-user-row">
							<CoverArt
								...
								imgClass="feed-avatar"
							></CoverArt>
							{item.user_id == sessionUserId ? 'You' : item.display_name} commented on {item.collection_owner_id ==
							sessionUserId
								? 'your'
								: 'the'} collection {item.collection_title}
						</div>
						{@render feedItemTag(item)}
					</div>
				</a>
				...
				>
					<div class="feed-item">
						<div class="feed-item-two-liner-user-row">
							...
							sessionUserId
								? 'your'
								: item.parent_post_display_name.concat(`'s`)} comment on the collection {item.collection_title}
						</div>
						{@render feedItemTag(item)}
					</div>
				</a>
			...
			{:else if item?.item_type == 'reaction' && !item?.collection_id}
				<a
					href={item.reaction_post_type == 'now_playing'
						? `/posts/${item.reaction_post_username}/now-playing/${parseTimestamp(item.reaction_post_created_at)}`
						: `/posts/${item.parent_post_username}/now-playing/${parseTimestamp(item.parent_post_created_at)}#${item.reaction_post_username?.concat(parseTimestamp(item.reaction_post_created_at))}`}
				>
					<div class="feed-item">
						<div class="feed-item-two-liner-user-row">
							<CoverArt
							...
							{item.user_id == sessionUserId ? 'You' : item.display_name} liked {item.reaction_post_user_id ==
							sessionUserId
								? 'your'
								: item.reaction_post_display_name.concat(`'s`)}
							{item.reaction_post_type == 'now_playing' ? 'post' : 'reply'}
							{item.artist_name || item.user_added_artist_name ? 'about' : ''}
							{item.parent_post_artist_name || item.parent_post_user_added_artist_name
								? 'on a post about'
								: ''}
						</div>
						{@render feedItemTag(item)}
			...
			{:else if item?.item_type == 'reaction' && item?.collection_id && item?.reaction_post_created_at}
				<a
					href={`/collection/${item.collection_id}#${item.reaction_post_username?.concat(parseTimestamp(item.reaction_post_created_at))}`}
				>
					<div class="feed-item">
						<div class="feed-item-two-liner-user-row">
							<CoverArt
				...
				<a href={`/collection/${item.collection_id}`}>
					<div class="feed-item">
						<div class="feed-item-one-liner">
							...
							></CoverArt>
							<span class="blurb">
								{item.user_id == sessionUserId ? 'You' : item.display_name} liked {item.collection_owner_id ==
								sessionUserId
									? 'your'
									: 'the'} collection
			...
			{:else if item?.item_type == 'collection_follow'}
				<a href={`/collection/${item.collection_id}`}>
					<div
						class={type == 'collections' ? 'collection-feed-item-one-liner' : 'feed-item-one-liner'}
					>
						<CoverArt
							item={avatarItem(item)}
							altText={`${item.display_name}'s avatar`}
...
<div class="feed-panel">
	{#if showFilters}
		<PanelHeader>
			...
			{/snippet}
			{#snippet button()}
				<OptionsMenu triggerText="filter" {optionsGroups} inputGroup="selected-options"
				></OptionsMenu>
			{/snippet}
		</PanelHeader>
	{:else}
	...
	>
		<input type="hidden" name="feed-mode" id="feed-mode" value={feedMode} />
		{#if remaining && remaining > 0}
			<div class="button-spacer">
				<button class="standard" formaction="?/loadMore" disabled={loadingMore}> load more </button>
			</div>
		{/if}
	</form>
	{#if mode == 'mini'}
