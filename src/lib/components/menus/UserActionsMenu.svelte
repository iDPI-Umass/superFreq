<script lang="ts">
	...
	const dialogTextOptions: App.Lookup = {
		blockUser:
			'This user will no longer be able to see or interact with your profile data, posts, or private collections. They will be able to see your public and open collections.',
		...
		unblockUser:
			'This grants this user the same permission to see your activity that everyone else has.'
	};
...
<Popover.Root bind:open={popOverOpenState}>
	...
	<Popover.Content transition={flyAndScale}>
		{#if mode == 'profileMenu'}
			{#if !blocked}
				<button class="popover-item" onclick={() => openDialog('blockUser')}>
					<Ban size="16" color="var(--freq-color-text-muted)"></Ban>
					<span class="descriptor"> block user </span>
				</button>
			{:else if blocked}
				<button class="popover-item" onclick={() => openDialog('unblockUser')}>
					<Circle size="16" color="var(--freq-color-text-muted)"></Circle>
					<span class="descriptor"> unblock user </span>
				</button>
			{/if}
			{#if !flagged}
				<button class="popover-item" onclick={() => openDialog('reportUser')}>
					<Flag size="16" color="var(--freq-color-text-muted)"></Flag>
					<span class="descriptor"> report user </span>
				</button>
			{:else if flagged}
				<button class="popover-item" onclick={() => openDialog('reportUser')} disabled>
					<Flag size="16" color="var(--freq-color-text-muted)"></Flag>
					<span class="descriptor"> user reported </span>
				</button>
			{/if}
		{:else if mode == 'sessionUserPostMenu'}
			<button class="popover-item" onclick={toggleEditState}>
				...
				<span class="descriptor"> edit </span>
			</button>
			<button class="popover-item" onclick={() => openDialog('deletePost')}>
				<Trash2 size="16" color="var(--freq-color-text-muted)"></Trash2>
				<span class="descriptor"> delete </span>
			</button>
		{:else if (mode = 'postMenu')}
			<button class="popover-item" onclick={() => openDialog('flagPost')}>
				<Flag size="16" color="var(--freq-color-text-muted)"></Flag>
				<span class="descriptor"> flag post </span>
			</button>
		{/if}
	</Popover.Content>
</Popover.Root>
...
<dialog aria-label="modal" bind:this={dialog} onclose={() => (showModal = false)}>
	<form method="POST" id={formIDs[dialogMode]} action={formActions[dialogMode]} use:enhance>
		<input type="hidden" id="profile-user-id" name="profile-user-id" value={profileUserId} />
		<input type="hidden" id="post-id" name="post-id" value={postId} form={formIDs[dialogMode]} />
		{#if success == null}
		...
		{:else if success == true}
			...
			<div class="dialog-options">
				<button aria-label="close modal" formmethod="dialog" class="standard" onclick={closeDialog}>
					close
				</button>
			</div>
