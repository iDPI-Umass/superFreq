...
<Popover.Root closeOnEscape={true} closeOnOutsideClick={true} bind:open={popOverOpenState}>
	<Popover.Trigger>
...
<dialog aria-label="modal" bind:this={dialog} onclose={() => (showModal = false)}>
	...
	<div class="delete-submit-options">
		<button aria-label="close modal" formmethod="dialog" onclick={closeDialog}> cancel </button>
		<button aria-label="close modal" formmethod="dialog" onclick={closeDialog}> delete </button>
	</div>
</dialog>
