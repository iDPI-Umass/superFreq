<script lang="ts">
    ...
    const permalink = `/posts/${reply.original_poster_username}/now-playing/${reply.original_post_date.toISOString()}#${reply.username?.concat(reply.created_at.toISOString())}`
