<script lang="ts">
	...
	let permalink = $derived(
		permalinkRoot.concat(`#${reply.username?.concat(permalinkTimestamp)}`)
	) as string;

	const parentPostTimestampString = reply?.parent_post_created_at
		? reply?.parent_post_created_at.toISOString()
		: null;
...
<div class="comment">
	<div class="comment-metadata">
		<div class="row-group-user-data">
			<CoverArt item={reply} altText={`${reply.display_name}'s avatar`} imgClass="avatar"
			></CoverArt>
	...
	<div class="comment-text">
		...
		{:else}
			<EditPostBody postData={reply} bind:editState></EditPostBody>
		{/if}
	</div>
			...
			<div class="row-group-icons">
				...
				{#if allowReply}
					<button class="like" onclick={() => (showPostReplyEditor = !showPostReplyEditor)}>
						...
						<span class="descriptor"> reply </span>
					</button>
				{/if}
			</div>
...
{#if showPostReplyEditor}
	<PostReplyEditor {reply}></PostReplyEditor>
{/if}
