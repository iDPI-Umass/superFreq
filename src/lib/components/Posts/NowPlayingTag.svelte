<script lang="ts">
	...
	let { artistName, itemTitle, itemType }: ComponentProps = $props();
</script>

{#if itemType == 'release_group'}
	<span class="now-playing-text" aria-details="Now Playing album {itemTitle} by {artistName}">
		<Disc size="16" color="var(--freq-color-text-medium-dark)"></Disc>
		{artistName} • {itemTitle}
		<Disc size="16" color="var(--freq-color-text-medium-dark)"></Disc>
	</span>
{:else if itemType == 'recording'}
	<span class="now-playing-text" aria-details="Now Playing track {itemTitle} by {artistName}">
		<MusicNote size="16" color="var(--freq-color-text-medium-dark)"></MusicNote>
		{artistName} • {itemTitle}
		<MusicNote size="16" color="var(--freq-color-text-medium-dark)"></MusicNote>
	</span>
{:else if itemType == 'episode'}
	<span class="now-playing-text" aria-details="Now Playing mix {itemTitle} by {artistName}">
		<BoomBox size="16" color="var(--freq-color-text-medium-dark)"></BoomBox>
		{artistName} • {itemTitle}
		<BoomBox size="16" color="var(--freq-color-text-medium-dark)"></BoomBox>
	</span>
...
<style>
	.profile-avatar {
		...
		margin: 0 auto;
		max-width: inherit;
		padding: var(--freq-spacing-2x-small) var(--freq-spacing-3x-small);
		margin-top: var(--freq-spacing-small);
