<script lang="ts">
	...
	let reactionActive = $derived(
		post?.reaction_user_ids ? post?.reaction_user_ids.includes(sessionUserId) : null
	) as boolean;
	...
	<div class="double-border">
		...
		<div class="post-body">
			{#if artistName}
				<NowPlayingTag
					{artistName}
					itemTitle={post.recording_name ??
						post.user_added_recording_name ??
						post.release_group_name ??
			...
			{:else}
				<EditPostBody postData={post} bind:editState></EditPostBody>
			{/if}
			<!-- {#if formData == true }
                <p>edited!</p>
			...
			{#if embedInfo?.id != null}
				<ListenEmbed {embedInfo}></ListenEmbed>
			{/if}
		</div>
		<div class="post-row">
			{#if mode != 'sample'}
				<div class="row-group-icons">
					<LikeReact {postId} {reactionActive} {reactionCount}></LikeReact>
					{#if mode == 'feed'}
						<a href={permalink}>
