...
<div class="search-bar">
	<ListModal bind:showModal>
		{#snippet headerText()}
			...
			{:else}
				Results for {searchResults.category} search
				<span class="dialog-header">{searchResults.query}</span>
			{/if}
		{/snippet}
		{#snippet list()}
			<div>
	...
	>
		<button type="submit" class="double-border-top" disabled={!validQuery || loading}>
		...
		<div class="input-sizing">
			<input
				...
				bind:value={query}
			/>
			<input type="hidden" name="results-limit" id="results-limit" value={limit} />
			<input type="hidden" name="search-category" id="search-category" value={searchCategory} />
			<input type="hidden" name="query-type" id="query-type" value="title" />
		</div>
	</form>
</div>
