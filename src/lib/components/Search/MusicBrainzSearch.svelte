<script lang="ts">
	...
	import {
		...
		artistOrigin,
		getArtistImage
	} from '$lib/resources/musicbrainz';
	...
	async function addItem(mode: string, item: App.RowData) {
		...
		if (mode == 'single') {
			...
			if (searchCategory == 'release_groups' || searchCategory == 'recordings') {
				...
				const thisItemIndex = collectionData.collectionItems.findIndex(
					(item) => item['artist_mbid'] == thisArtistMbid
				);
				collectionData.collectionItems[thisItemIndex]['artist_discogs_img_url'] = artistImgUrl;
				promiseStates.imgPromise = new Promise((resolve) => resolve(success));
			}
		...
		if (mode == 'collection') {
			promiseStates.continueClientSideImgPromise = false;
			const collectionItems = await addCollectionItemNoImg(
				item,
				collectionData.collectionItems,
				collectionData.deletedItems,
				collectionLimit,
				searchCategory,
				mbidCategory
			);
			...
			if (searchCategory == 'release_groups' || searchCategory == 'recordings') {
				...
				const { success, coverArtArchiveUrl, lastFmCoverArtUrl } = await getCoverArt(releaseGroup);
				const thisItemIndex = collectionData.collectionItems.findIndex(
					(item) => item['release_group_mbid'] == releaseGroup.release_group_mbid
				);
				collectionData.collectionItems[thisItemIndex]['artist_discogs_img_url'] = artistImgUrl;
				collectionData.collectionItems[thisItemIndex]['img_url'] = success
					? coverArtArchiveUrl
					: null;
				collectionData.collectionItems[thisItemIndex]['last_fm_img_url'] = success
					? lastFmCoverArtUrl
					: null;
				promiseStates.imgPromise = new Promise((resolve) => resolve(success));
			} else if (searchCategory == 'artists') {
								...
								{#if searchCategory == 'release_groups' || searchCategory == 'recordings'}
									<div class="result-image">
										<CoverArt
											...
											altText="album {releaseGroupName(searchCategory, item)} by artist {artistName(
												searchCategory,
												item
											)}"
