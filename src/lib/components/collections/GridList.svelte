...
<script lang="ts">
	...
	const undersizedCollection = $derived(
		(mode == 'view' && layout == 'grid' && collectionData.collectionItems.length < 6) ||
			(layout == 'condensed-grid' && collectionData.collectionItems.length < 4)
			? true
			: false
	);
...
{#snippet editorInteractions(item: any, labelText: string)}
	<div class="editor-interactions">
		<button class="standard" onclick={() => deleteItem(item)}> x remove </button>
...
{#snippet coverArt(item: any, itemType: string, mode: string)}
	...
	{:else if itemType.includes('release_group') && mode == 'view'}
		<CoverArt {item} altText={item['release_group_name']}></CoverArt>
	...
	{:else if itemType.includes('recording') && mode == 'view'}
		<CoverArt {item} altText={item['recording_name']} imgClass="grid-list-img"></CoverArt>
	{:else if itemType.includes('episode')}
		<CoverArt {item} altText={item['episode_title']} imgClass="grid-list-img"></CoverArt>
	{:else if itemType.includes('artist') && mode == 'view'}
		<CoverArt {item} altText={item['artist_name']} imgClass="grid-list-img"></CoverArt>
	...
	{:else if itemType == 'collection' && mode == 'view'}
		<div class="grid-list-image-stack">
			<CollectionImageTrio collection={item} orientation="diagonal-stack"></CollectionImageTrio>
		</div>
	{/if}
{/snippet}
...
{#snippet metadataBlurb(item: any, itemType: string, mode: string)}
	{#if itemType.includes('artist')}
		<CollectionItemTag display={showTags} {itemType}></CollectionItemTag>
		<span class="artist">
	...
	{:else if itemType.includes('collection')}
		...
		<span class="artist">
			{#if mode == 'view'}
				<a
					href="/user/{item['connected_collection_owner_username'] ?? item['collection_username']}"
				>
					by {item['connected_collection_owner_display_name'] ?? item['collection_display_name']} ({item[
						'connected_collection_owner_username'
					] ?? item['collection_username']})
				</a>
			{:else}
...
{#await collectionData.collectionItems.length > 0 then}
	...
	{:else if mode == 'view'}
		<ul aria-label="collection items" class={format[layout][0]}>
