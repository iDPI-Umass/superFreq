...
<form method="POST" action="?/getCollectionList" use:enhance>
	<input type="hidden" id="post-id" name="post-id" value={postId} />
	<button class="like" type="submit">
		<div class="row-group-icon-description">
			<Save class="icon" size="16" color="var(--freq-color-text-muted)"></Save>
			<span class="descriptor"> Save </span>
		</div>
	</button>
</form>

<ListModal bind:showModal={showCollectionsListModal}>
	{#snippet headerText()}
					...
					<div class="list-modal-li-row">
						<div class="list-modal-li-row-button-spacing">
							...
							>
								...
								/>
								<button class="add" type="submit" disabled={addingItem}> + save </button>
							</form>
						</div>
						{collection.title}
					</div>
...
<NotificationModal bind:showModal={showSuccessModal}>
	{#snippet headerText()}
		Success!
	{/snippet}
	{#snippet message()}
