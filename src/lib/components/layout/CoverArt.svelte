...
<script lang="ts">
	...
	import {
		checkFetchedCoverArt,
		getLastFmCoverArt,
		getCoverArt,
		getCoverArtClientSide
	...
	const coverArtItem = $derived({
		...
		release_group_name:
			releaseGroupName ?? item?.release_group_name ?? item?.avatar_release_group_name ?? null,
	...
	const imageSelector = function (coverArtItem: App.RowData) {
		const validUrl =
			coverArtItem['last_fm_img_url'] ||
			coverArtItem['img_url'] ||
			coverArtItem['artist_discogs_img_url']
				? true
				: false;
		let url = '';
		...
		if (
			coverArtItem['artist_discogs_img_url'] &&
			!(coverArtItem['last_fm_img_url'] || coverArtItem['img_url'])
		) {
...
{:else if !validUrl && clientSideLoad}
	...
	{:then result}
		<img
			src={result.lastFmCoverArtUrl ?? result.coverArtArchiveUrl ?? result.wave}
			alt={result ? altText : 'not found'}
			class={imgClass}
		/>
	{:catch}
