...
export const selectAllOpenPublicCollections = async function (
	batchSize: number,
	...
	const selectCollections = await db.transaction().execute(async (trx) => {
		const collectionsCount = await trx
			...
			.where(({ eb, or }) => or([eb('status', '=', 'open'), eb('status', '=', 'public')]))
...
export const selectRecentOpenPublicCollections = async function (
	batchSize: number,
	batchIterator: number
) {
	const selectCollections = await db.transaction().execute(async (trx) => {
		const selectCollectionsMetadata = await trx
			...
			.select(['collection_id', 'title', 'username', 'display_name', 'updated_at'])
		...
		for (const collection of collections) {
			...
			const collectionImages = await trx
				...
				.select(['original_id', 'img_url', 'last_fm_img_url', 'artist_name', 'release_group_name'])
...
export const selectSpotlightCollections = async function (
	spotlightCollectionId: string,
	batchSize: number
) {
	const selectCollections = await db.transaction().execute(async (trx) => {
		const selectCollectionsMetadata = await trx
...
export const selectFollowedUsersOpenPublicCollections = async function (
...
export const selectListProfileUserViewableCollections = async function (
	sessionUserId: string,
	username: string
) {
	const selectCollections = await db.transaction().execute(async (trx) => {
		...
		if (sessionUserId == profileUserId) {
			selectInfo = await trx
				...
				.select(['collection_id', 'title', 'updated_at', 'created_at', 'display_name'])
		...
		for (const collection of collections) {
			...
			const collectionImageTrio = await trx
				.selectFrom('collections')
				.select(['img_url', 'last_fm_img_url', 'artist_name', 'release_group_name'])
				.where('collection_id', '=', collectionId)
...
export const selectListProfileUserFollowingCollections = async function (username: string) {
	const selectCollections = await db.transaction().execute(async (trx) => {
		...
		const selectCollections = await trx
			.selectFrom('collection_metadata')
			.select(['collection_id', 'title', 'updated_at', 'created_at', 'username', 'display_name'])
...
export const selectViewableCollectionContents = async function (
	collectionId: string,
	sessionUserId: string,
	limit = 50,
	iterator = 0
	...
	const selectCollection = await db.transaction().execute(async (trx) => {
		try {
			...
			if (
				status == 'private' &&
				!(owner_id == sessionUserId || collaborators.includes(sessionUserId))
			) {
	...
	const editPermission =
		status == 'open' || owner_id == sessionUserId || collaborators.includes(sessionUserId)
			? true
			: false;
...
export const selectEditableCollectionContents = async function (
	collectionId: string,
	sessionUserId: string
) {
	const selectCollection = await db.transaction().execute(async (trx) => {
		try {
			...
			if ((status == 'public' || status == 'private ') && owner_id != sessionUserId) {
	...
	if (!editPermission) {
		return {
			...
			collectionMetadata: null,
			collectionContents: null,
			deletedCollectionContents: null
		};
	}
	...
	const selectCollection = await db.transaction().execute(async (trx) => {
		try {
			...
			const selectCollection = await trx
				.selectFrom('collections_info as info')
				.innerJoin(
					'collections_contents as contents',
				...
				.leftJoin(
					'user_added_metadata',
					'user_added_metadata.id',
					'contents.user_added_metadata_id'
				)
				.select([
...
export const insertCollection = async function (
	sessionUserId: string,
	...
	const { artistsMetadata, releaseGroupsMetadata, recordingsMetadata } =
		await prepareMusicMetadataInsert(collectionItems);
	...
	const insert = await db.transaction().execute(async (trx) => {
		...
		for (const row of userAddedMetadataRows) {
			...
			const collectionItemIndex = collectionItems.findIndex(
				(item) =>
		...
		const collectionContents = await populateCollectionContents(
			sessionUserId,
			collectionItems,
			collectionId
		);
...
export const insertUpdateTopAlbumsCollection = async function (
	sessionUserId: string,
	collectionItems: App.RowData[]
) {
	const timestampISOString: string = new Date().toISOString();
	...
	const { artistsMetadata, releaseGroupsMetadata } =
		await prepareMusicMetadataInsert(collectionItems);
...
export const deleteCollection = async function (sessionUserId: string, collectionId: string) {
	...
	const update = await db.transaction().execute(async (trx) => {
		try {
			const selectCollection = await trx
				.selectFrom('collections_info')
				.select(['collection_id', 'status', 'changelog'])
				.where('collection_id', '=', collectionId)
				.where('owner_id', '=', sessionUserId)
				.executeTakeFirstOrThrow();
...
export const saveItemToCollection = async function (
	sessionUserId: string,
	itemId: string,
	collectionId: string
) {
	...
	const update = await db.transaction().execute(async (trx) => {
		try {
			const collectionInfo = await trx
				...
				.select(['info.collection_id as id', 'info.changelog as changelog'])
				.where(({ eb, and, or }) =>
					and([
			...
			const { isDuplicate, duplicateItem } = checkDuplicate(
				item[itemMbidCategory] ?? item['user_added_metadata_id'],
				activeItems,
				deletedItems,
				itemMbidCategory
			);
