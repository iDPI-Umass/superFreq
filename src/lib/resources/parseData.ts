...
export const prepareMusicDataUpsert = function (
...
export const populateCollectionContents = function (
...
export const listenUrlSourceWhitelist = ['bandcamp', 'soundcloud', 'youtube', 'mixcloud'];
...
export const parseBandcampHtml = async function (listenUrlString: string) {
	...
	const embedAccount = document.head.querySelector('meta[property=og:site_name]')
		?.content as string;
...
export const getListenUrlData = async function (listenUrlString: string) {
	...
	async function parseSoundcloudHtml(html: any) {
		const { document } = await parseHTML(html);
		const embedLink = document.head.querySelector(
	...
	async function parseYouTubeHtml(html: any, listenUrlString: string) {
		let itemId = listenUrlString.includes('youtu.be')
			? listenUrlString.split('/')[3].split('?')[0]
			: listenUrlString.split('=')[1];
