...
export const selectFeedData = async function (
		...
		const feedData = await trx
			...
			.where(({ eb, or, and, not }) =>
				and([
					...
					not(and([eb('item_type', '=', 'social_follow'), eb('user_id', '=', sessionUserId)])),
					not(
						and([eb('item_type', '=', 'social_follow'), eb('target_user_id', '!=', sessionUserId)])
					),
					not(
						and([
...
export const selectFollowingFeed = async function (
	sessionUserId: string,
	batchSize: number,
	batchIterator: number,
	timestampStart: Date,
...
export const selectFirehoseFeed = async function (
	sessionUserId: string,
	batchSize: number,
	batchIterator: number,
	timestampStart: Date,
...
export const selectNotificationsFeed = async function (
	sessionUserId: string,
	batchSize: number,
	batchIterator: number,
	timestampStart: Date,
