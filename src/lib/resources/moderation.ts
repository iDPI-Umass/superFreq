	...
	const selectQueueItems = await db.transaction().execute(async (trx) => {
		try {
			...
			const queueItems = await trx
				...
				.leftJoin(
					'profiles as target_user_profile',
					'actions.target_user_id',
...
export const updateModerationItem = async function (
	sessionUserId: string,
	moderationItem: App.RowData
) {
...
export const updateBugReport = async function (
	sessionUserId: string,
	bugReportUpdate: App.RowData
) {
	const timestampISOString: string = new Date().toISOString();
