...
export const searchCollections = async function (
	query: string,
	...
	if (queryType == 'title') {
		...
		const search = await db
			...
			.select([
				...
				'updated_at',
				'created_at',
				sql`GREATEST(word_similarity(${query}, display_name), word_similarity(${query}, username))`.as(
					'rank'
				)
			])
		...
		.union(
			db
				...
				.select([
					...
					sql<number>`GREATEST(word_similarity(${query}, display_name), word_similarity(${query}, username))`.as(
						'rank'
					)
				])
				.where(sql<boolean>`${query} <% username OR ${query} <% display_name`)
