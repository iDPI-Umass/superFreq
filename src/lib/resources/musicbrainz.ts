...
import {
	PUBLIC_LAST_FM_API_KEY,
	PUBLIC_DISCOGS_TOKEN,
	PUBLIC_WIKIDATA_TOKEN
...
export const musicbrainzMetadataRelationUrl = function (
	relations: App.RowData[],
	relationType: string
) {
	...
	const thisRelation =
		(relations.find((element) => element.type == relationType) as App.RowData[]) ?? null;
...
export const getWikipediaUrlFromWikidata = async function (
...
export const mbSearch = async function (
	query: string,
...
export const musicbrainzAdvancedSearch = async function (
	category: string,
	searchTerms: any,
	limit: number = 25
) {
...
export const getLabel = async function (
	searchCategory: string,
...
export const remixerMbid = function (searchCategory: string, item: App.RowData) {
	...
	if (searchCategory == 'recordings') {
		mbid =
			item['releations'] && item['relations'][0]['artist']['type'] == 'remixer'
				? item['relations'][0]['artist']['id']
				: null;
	}
	return mbid;
};
...
export const getCoverArt = async function (releaseGroup: App.RowData) {
	const validReleaseGroup =
		releaseGroup.release_group_name == null && releaseGroup.release_group_mbid == null
			? false
			: true;
...
export const getCoverArtClientSide = async function (
	releaseGroup: App.RowData,
	...
	const coverArtArchiveUrl = releaseGroup.release_group_mbid
		? `https://coverartarchive.org/release-group/${releaseGroup.release_group_mbid}/front`
		: null;
...
export const checkDuplicate = function (
	mbid: string,
	addedItems: App.RowData | App.RowData[],
	...
	const findUserAddedMetadataDuplicate = addedItems.find(
		(element) => element['user_added_metadata_id'] == mbid
	);
...
export const checkDeleted = function (
	item: App.RowData,
	deletedItems: App.RowData | App.RowData[],
	mbidCategory: string
) {
	...
	const wasDeleted = deletedItems.find((element) => element[mbidCategory] == itemMbid)
		? true
		: false;
	return wasDeleted;
};
...
export const addCollectionItem = async function (
	...
	if (limitReached) {
		alert(
			`Only ${limit} items are allowed in this collection. Please delete an item before you add something new.`
		);
		return {
			addedItems,
			deletedItems,
