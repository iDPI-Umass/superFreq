...
export const selectPost = async function (
	sessionUserId: string,
	username: string,
				...
				.innerJoin(
					(eb) =>
						eb
							...
							.as('reactions'),
					(join) => join.onRef('reactions.post_id', '=', 'posts.id')
				)
				.select([
					'posts.id as id',
					'posts.text as text',
...
export const selectPostAndReplies = async function (
	sessionUserId: string,
	username: string,
	timestampString: string
) {
...
export const selectUserPostsSample = async function (
	sessionUserId: string,
	username: string,
	batchSize: number,
	...
	const selectPosts = await db.transaction().execute(async (trx) => {
		try {
			await trx
				...
				.selectAll()
				.where(({ eb, and }) =>
					and([eb('target_user_id', '=', sessionUserId), eb('username', '=', username)])
				)
				.executeTakeFirstOrThrow();
...
export const getReactionData = async function (post_id: string, user_id: string) {
	const getReactionData = await db
		.with('permissioned_collection', (db) =>
			db
				.selectFrom('reactions')
				.selectAll()
		...
		.select(({ fn, eb }) => [
			...
			eb.selectFrom('reactions').selectAll().where('user_id', '=', user_id).as('user_reaction')
		])
		.execute();
