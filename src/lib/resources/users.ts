...
export const selectProfilePageData = async function (
	...
	const selectProfileData = await db.transaction().execute(async (trx) => {
		...
		if (sessionUserBlockInfo) {
			return {
				profileUserData,
				profileUserBlockInfo: null,
				profileUserFlagInfo: null,
				collectionCount: null,
				collectionFollowingCount: null,
				userFollowingCount: null,
				nowPlayingPostsCount: null,
				topAlbumsCollection: null,
				followInfo: null,
				permission: false
			};
		}
		...
		const countCollections = await trx
			...
			.where(({ eb, and, or, not }) =>
				and([
					...
					or([eb('social.user_role', '=', 'owner'), eb('social.user_role', '=', 'collaborator')]),
		...
		const topAlbumsCollection = await trx
			...
			.leftJoin(
				...
				'collections_contents.release_group_mbid'
			)
			.leftJoin(
				'user_added_metadata',
				'user_added_metadata.id',
				'collections_contents.user_added_metadata_id'
			)
			.select([
			...
			.where(
				'collections_contents.collection_id',
				'=',
				profileUserData?.top_albums_collection_id as string
			)
	...
	const invite = await db.transaction().execute(async (trx) => {
		try {
			const selectInvite = await trx
				.selectFrom('invites')
				.select(['email', 'approved', 'user_id'])
				.where('email', '=', email)
				.executeTakeFirstOrThrow();

			const { approved, user_id }: { approved: boolean | null; user_id: string | null } =
				await selectInvite;
...
export const newSessionProfile = async function (
	sessionUserId: string,
	profileData: App.RowData,
	email: string,
...
export const updateSessionProfile = async function (
	sessionUserId: string,
	profileData: App.RowData,
	avatarItem: App.RowData
) {
...
export const insertUpdateUserFollow = async function (
	sessionUserId: string,
...
export const insertUpdateCollectionFollow = async function (
	sessionUserId: string,
	collectionId: string
) {
	const timestampISOString: string = new Date().toISOString();
