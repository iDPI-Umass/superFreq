...
export const load: PageServerLoad = async ({ url, locals: { safeGetSession } }) => {
	...
	if (loadData) {
		...
		if (batchIterator == 0 || feedMode == 'following') {
			const select = await selectFeedData(
				sessionUserId,
				batchSize,
				batchIterator,
				timestampStart,
		...
		if (batchIterator == 0 || feedMode == 'discover') {
			const selectFirehose = await selectFirehoseFeed(
				sessionUserId,
				batchSize,
				batchIterator,
				timestampStart,
...
export const actions = {
	...
	editPost: async ({ request, locals: { safeGetSession } }) => {
		...
		const feedItemIndex =
			feedData?.feedItems.findIndex((element) => element.post_id == updateData.post_id) ?? null;
		if (feedItemIndex >= 0) {
	...
	deletePost: async ({ request, locals: { safeGetSession } }) => {
		...
		const permalink = parentPostId
			? `/posts/${parentPostUsername}/now-playing/${parentPostTimestamp}`
			: '/';
