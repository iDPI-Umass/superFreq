...
import {
	insertUpdateTopAlbumsCollection,
	selectEditableTopAlbumsCollection
...
export const load: PageServerLoad = async ({ locals: { safeGetSession } }) => {
	...
	const { collectionContents, deletedCollectionContents } =
		await selectEditableTopAlbumsCollection(sessionUserId);
...
export const actions = {
	submitCollection: async ({ request, locals: { safeGetSession } }) => {
		...
		const insertUpdate = await insertUpdateTopAlbumsCollection(
			sessionUserId,
			activeAndDeletedCollectionItems
		);
