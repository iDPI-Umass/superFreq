<script lang="ts">
	...
	let { profileUserData, followInfo, permission, profileUserBlockInfo, profileUserFlagInfo } =
		$derived(profileData);
	...
	$effect(() => {
		...
		viewProfile.metrics.collectionCount = permission
			? (profileData?.collectionCount as number)
			: null;
		viewProfile.metrics.collectionFollowingCount = permission
			? (profileData?.collectionFollowingCount as number)
			: null;
		viewProfile.metrics.userFollowingCount = permission
			? (profileData?.userFollowingCount as number)
			: null;
		viewProfile.metrics.nowPlayingPostsCount = permission
			? (profileData?.nowPlayingPostsCount as number)
			: null;
		viewProfile.topAlbumsCollection = permission
			? (profileData?.topAlbumsCollection.slice(0, 8) as App.ProfileObject[])
			: [];
		...
		<div class="avatar-user-data">
			<div class="cover-art-widget">
				...
				{#if isSessionUserProfile}
					<button class="mini" onclick={() => goto('/account')}> edit </button>
				{/if}
			</div>
		...
		<div class="profile-buttons-group">
			{#if isSessionUserProfile}
				<button class="double-border-top" onclick={() => goto('/account')}>
					<div class="inner-border-condensed">edit profile</div>
				</button>
			{:else}
				<form
					method="POST"
				...
				>
					...
					/>
					<button class="standard" type="submit" formaction="?/followUser" disabled={followLoading}>
		...
		<div class="profile-stats-box" aria-label="user metrics">
			<div class="metric" aria-label="metric">
				<a class="metrics" href="/user/{viewProfile.username}/collections">
					...
					<p class="data-muted-uppercase">collections</p>
				</a>
			</div>
			<div class="metric" aria-label="metric">
				<a class="metrics" href="/user/{viewProfile.username}/now-playing-posts">
					...
					<p class="data-muted-uppercase">posts</p>
				</a>
			</div>
			<div class="metric" aria-label="metric">
				<a class="metrics" href="/user/{viewProfile.username}/collections-following">
					...
					<p class="data-muted-uppercase">collections followed</p>
				</a>
			</div>
			<div class="metric" aria-label="metric">
				<a class="metrics" href="/user/{viewProfile.username}/users-following">
					...
					<p class="data-muted-uppercase">users followed</p>
				</a>
			</div>
		</div>
	</div>
...
<div class="content">
				...
				{#snippet button()}
					<span>
						{#if viewProfile.user_id == sessionUserId}
							<button class="standard" onclick={() => goto(`/user/top-albums`)}> edit </button>
						{/if}
					</span>
				{/snippet}
			</PanelHeader>
	...
	{#if viewProfile.user_id == sessionUserId}
		...
		<NewNowPlayingPost {parsedUrlInfo}></NewNowPlayingPost>
		<Feed
			{sessionUserId}
			mode="feed"
