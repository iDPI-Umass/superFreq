<script lang="ts">
	...
	function selectHeaderImage(discogsData: App.RowData | null) {
		...
		if (category == 'artist') {
			imgUrl = discogsData.artist.images.find((element) => element.type == 'primary')[
				'resource_url'
			];
		} else if (category == 'album') {
			imgUrl = discogsData.release.images.find((element) => element.type == 'primary')[
				'resource_url'
			];
			...
			imgUrl = discogsData.label.images.find((element) => element.type == 'primary')[
				'resource_url'
			];
		}
	...
	<div class="column-half">
		...
		<div>
			...
			{:else if category == 'track'}
				<p>
					Appears on <a
						href="/music/album/{musicbrainzMetadata['releases'][0]['release-group']['id']}"
						>{musicbrainzMetadata['releases'][0]['title']}</a
					>
				</p>
	...
	<div class="column-half">
		{#if wikipediaExtract}
			<div class="data-breakout">
				...
				<span class="link"
					><a class="read-more" href={wikipediaExtract.url}>Continue reading at Wikipedia</a></span
				>
				<p>
					Wikipedia content provided under the terms of the <a
						href="https://creativecommons.org/licenses/by-sa/3.0/">Creative Commons BY-SA license</a
					>
				</p>
			</div>
		{/if}
