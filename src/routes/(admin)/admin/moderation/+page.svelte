				...
				<tr>
					...
					<td>
						{#if item.target_post_timestamp}
							{#if Object.keys(item.post_changelog).length > 0}
								<button class="more-info" onclick={() => (showChangelog = true)}>
									post changelog
								</button>
								<NotificationModal showModal={showChangelog}>
									{#snippet headerText()}
										Post Changelog
									{/snippet}
									{#snippet message()}
							...
							{/if}
							<a
								href="/posts/{item.target_post_username}/now-playing/{parseTimestamp(
									item.target_post_timestamp
								)}"
							>
					...
					<td>
						{#if Object.keys(item.moderation_log).length > 0}
							<button class="more-info" onclick={() => (showModerationLog = true)}>
								moderation log
							</button>
							<NotificationModal showModal={showModerationLog}>
								{#snippet headerText()}
									Moderation Log
								{/snippet}
								{#snippet message()}
						...
						>
							<input type="hidden" name="item-id" id="item-id" value={item.moderation_item_id} />
							<input type="hidden" name="item-type" id="item-type" value={itemType(item)} />
							<textarea name="notes" id="notes"></textarea>
							<div class="cell-row">
								<div class="input-column">
									<label for="resolved"> archive </label>
