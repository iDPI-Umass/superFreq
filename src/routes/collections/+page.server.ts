...
import {
	selectSpotlightCollections,
	selectRecentOpenPublicCollections,
	selectFollowedUsersOpenPublicCollections
...
export const load: PageServerLoad = async ({ locals: { safeGetSession } }) => {
	...
	const selectCollectionsFeed = await selectFeedData(
		sessionUserId,
		10,
		0,
		feedTimestampStart,
		feedTimestampEnd,
		{ items: ['collection_follow', 'collection_edit'] }
	);
	const collectionsFeed = selectCollectionsFeed;

	const selectUsersCollections = await selectFollowedUsersOpenPublicCollections(
		sessionUserId,
		10,
		0
	);
